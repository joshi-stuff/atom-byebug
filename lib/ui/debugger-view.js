...
import CallStackPane from './call-stack-pane'
...
export default class DebuggerView extends Widget {
  constructor() {
    ...
    this._connection_state = this._child('.state .connection');
  ...
  set connected(connected) {
    if(connected) {
      this._connection_state.addClass('connected');
    } else {
      this._connection_state.removeClass('connected');
    }
  }
...
DebuggerView.HTML =
  ...
  <div class="toolbar">
    ...
    <button id="${DebuggerView.EVENT.CLEAR}" class="btn icon icon-remove-close">
      Clear output
    </button>
    <button id="${DebuggerView.EVENT.CONFIGURE}" class="btn icon icon-settings">
      Configure project
    </button>
  </div>
  ...
  <div class="status-bar">
    ...
    <div class="state">
      <span class="icon icon-radio-tower connection" title="Byebug connection status"></span>
    </div>
  </div>
  `;
